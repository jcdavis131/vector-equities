...
def main():
    ...
    sequences, _feat_to_idx_map_idx, _extra = build_sequences(
        Z,
        mask,
        Z_raw,
        tickers,
    ...
    for epoch in range(Args.epochs):
        ...
        for s in range(0, len(train_seqs), Args.batch):
            ...
            loss = 0.0
            # NCE temporal
            nce_loss, _n_pairs, _n_valid = temporal_info_nce(
                c_seq,
                batch_t["valid_mask"],
                batch_t["sector_ids"],
                temp=Args.temp,
    ...
    with torch.no_grad():
        ...
        for s in range(0, len(all_seqs), Args.batch):
            ...
            batch_np = build_batch(batch_list)
            batch_t = to_torch(batch_np)
            c_seq, _z_seq, out = model.forward_sequence(
                batch_t["xs_seq"],
                batch_t["ms_seq"],
                batch_t["time_enc_seq"],
                batch_t["year_norm_seq"],
            ...
            for b_idx, seq in enumerate(batch_list):
                ...
                for seq_pos in range(valid_len):
                    all_embs.append(c_seq[b_idx, seq_pos].cpu().numpy())
                    all_meta.append(
                        (
                            seq["ticker"],
                            int(seq["fiscal_years"][seq_pos]),
                            seq["sector"],
                        )
                    )
