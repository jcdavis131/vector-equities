...
import json, time, sys, os, subprocess, pathlib, random
    ...
    try:
        ...
        if os.path.exists(tmp) and os.path.getsize(tmp) > 5000:
            try:
                ...
                # try read raw
                print(f"  parse fail cik {cik_pad} size {os.path.getsize(tmp)} err {e}", flush=True)
                return None, code
        else:
            # check why
            size = os.path.getsize(tmp) if os.path.exists(tmp) else 0
            print(f"  curl no file or small cik {cik_pad} code {code} size {size} stderr {res.stderr[:200]}", flush=True)
            return None, code
    except Exception as e:
    ...
    for attempt in range(max_retries):
        ...
        if data:
            ...
            if code in ("429","403","503"):
                sleep_t += 2
            print(f"  retry {attempt+1}/{max_retries} cik {cik_pad} code {code} sleep {sleep_t:.1f}s", flush=True)
            time.sleep(sleep_t)
    return None, False
...
if __name__ == "__main__":
    ...
    for idx, entry in enumerate(subset):
        ...
        if success and data:
            ...
            print(f"[{idx+1}/{len(subset)}] {ticker} {cik} OK DEF14A:{def14a} Form4:{form4} total:{len(forms)}  running ok {ok}", flush=True)
            results.append({"cik":cik,"ticker":ticker,"ok":True,"def14a_recent":def14a,"form4_recent":form4})
        else:
            fail+=1
            print(f"[{idx+1}/{len(subset)}] {ticker} {cik} FAIL running fail {fail}", flush=True)
    ...
    print(f"Done {ok}/{len(subset)} fail {fail} final cache sub count {cnt_final}", flush=True)
