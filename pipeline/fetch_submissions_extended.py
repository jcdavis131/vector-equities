...
def fetch_json_file(cik_pad, file_name):
    ...
    if out.exists() and out.stat().st_size>5000:
        try:
            ...
            pass
    url=f"https://data.sec.gov/submissions/{cik_pad}_{file_name}" if "_" in file_name else f"https://data.sec.gov/submissions/{file_name}"
        ...
        for i, form in enumerate(forms):
            if "DEF 14A" in form:
                try:
                    ...
                    continue
                results.append((dates[i] if i < len(dates) else "", acc[i] if i < len(acc) else "", docs[i] if i < len(docs) else ""))
