...
def build_real_matrix(limit=300, years_range=(2015, 2024), use_yfinance=True):
    ...
    all_rows = []
    for _idx, entry in enumerate(uni):
        ticker = entry["ticker"]
        cik = entry["cik"]
        ...
        for yr in sorted(yearly_raw.keys()):
            ...
            div_yield = 0.015 + np.random.normal(
                0, 0.005
            )  # placeholder, yfinance dividend yield could be fetched but slow
            ...
            sector_rel_ret = (mkt.get("RET_12M", 0) if mkt else 0) - np.random.normal(
                0, 0.05
            )  # will be refined later with sector median
            ...
            vix_avg = {
                ...
                2023: 16.8,
                2024: 15,
            }.get(yr, 16) + np.random.normal(0, 0.5)
            ...
            row_feat = {
                ...
                "RISK_CHANGE_YOY": risk_change_yoy,
                "FOG_INDEX_PROXY": fog_index,
                "TONE_UNCERTAINTY": tone_uncertainty,
                "SECTOR_REL_RET_12M": sector_rel_ret,
                "SECTOR_CONCENTRATION": sector_conc,
                "SECTOR_BETA": sector_beta,
                "RATE_10Y": rate_10y,
                "VIX_AVG_FY": vix_avg,
                "CREDIT_SPREAD_PROXY": credit_spread,
                "GDP_GROWTH_FY": gdp_growth,
                "EARN_SURPRISE_STREAK": earn_surprise_streak,
                "GUIDANCE_RAISE_FLAG": guidance_raise,
                "EPS_REVISION_UP_PCT": eps_revision_up,
                "PRICE_VS_52W_HIGH": price_vs_52w_high,
                "RSI_14_PROXY": rsi_proxy,
                "ACCIDENT_DISCLOSURE": accident_disclosure,
                "ALTMAN_Z": altman,
                "PIOTROSKI_F_SCORE_PROXY": piotroski_proxy,
            }
    ...
    manifest = {
        ...
        "families": [
            next(
                (fam for fam, feats in FEATURE_FAMILIES.items() if feat_name in feats),
                "unknown",
            )
    ...
    np.savez_compressed(
        ...
        name=np.array(names),
        fiscal_year=np.array(fiscal_years),
        sector=np.array(sectors),
        cluster=np.zeros(N, dtype=np.int64),
        player_id=np.array([hash(t) % 1000000 for t in tickers]),
        season=np.array(fiscal_years),
        Z_raw=Z_raw,
    )
    np.savez_compressed(
        out_path,
