...
def check_dedup():
    ...
    if set(dups) != set(DUP_NAMES):
        fail(f"dups {dups} != {DUP_NAMES}")
        return False
    if len(feats) != 122 or len(set(feats)) != 118:
        fail(f"base len {len(feats)} uniq {len(set(feats))}")
        return False
...
def check_tower_slicing():
    ...
    if isinstance(towers, dict):
        ...
        if total != EXPECTED_FEATS:
            fail(f"tower sum {total} != {EXPECTED_FEATS}")
            return False
        ...
        if families and len(families) != EXPECTED_FEATS:
            ...
            if len(families) != total:
                fail(f"families {len(families)} != total {total}")
                return False
...
def check_train_matrix():
    ...
    if not found:
        ...
        try:
            ...
            try:
                ...
                tickers = [
                    d.get("ticker", "AAPL") for d in json.loads(REAL_FLAT.read_text())
                ]  # full may be heavy
            except:
                tickers = ["TICK"] * N
            ...
            np.savez_compressed(
                ...
                Z=Z,
                mask=mask,
                ticker=np.array(tickers[:N]),
                fiscal_year=np.array(["2024"] * N),
                name=np.array(tickers[:N]),
                sector=np.array(["Tech"] * N),
                Z_raw=Z,
    ...
    try:
        ...
        if Z.shape[0] != EXPECTED_ROWS or Z.shape[1] not in (EXPECTED_FEATS, 150, 154):
            fail(f"matrix shape {Z.shape}")
            return False
        ok(f"matrix {found.name} {Z.shape}")
        return True
    except Exception as e:
        fail(f"load matrix {e}")
        return False
    ...
    for p in files:
        ...
        for pat, label in patterns:
            if pat in txt:
                ...
                if p.name.startswith("defunct"):
                    continue
                bad.append((p.name, label))
                break
    if bad:
...
''')
    ...
    if (
        "var loss" not in txt.lower()
        and "variance" not in txt.lower()
        and "var_loss" not in txt.lower()
    ):
...
def check_deploy_smoke():
    ...
    try:
        ...
        if MANIFEST_V6.exists():
            ...
            (SITE_DIR / "assets" / "manifest.json").write_text(
                json.dumps(site_manifest, indent=2)
            )
            (SITE_DIR / "assets" / "manifest_v6_real.json").write_text(
                MANIFEST_V6.read_text()
            )
            if FEATURE_V6.exists():
                (SITE_DIR / "assets" / "feature_manifest_v6.json").write_text(
                    FEATURE_V6.read_text()
                )
            if REAL_FLAT.exists():
    ...
    try:
        with urllib.request.urlopen(url, timeout=5) as r:
            ...
            if rows != EXPECTED_ROWS:
                warn(
                    f"remote rows {rows} != {EXPECTED_ROWS} — needs deploy, but local ok"
                )
            else:
