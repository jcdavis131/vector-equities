...
def family_slices(manifest):
    ...
    for j, _f in enumerate(manifest["features"]):
...
def season_index(fiscal_years):
    uniq = sorted({str(s) for s in fiscal_years})
...
def main():
        ...
        for s in range(0, len(perm), args.batch):
            ...
            if game_z is not None:
                ...
                if valid_next.any():
                    ...
                    loss = loss + DEFAULT_WEIGHTS["next_profile"] * F.smooth_l1_loss(
                        out_a["next_profile"][valid_t], game_z[nxt_t]
                    )
            ...
            if ev_z is not None:
                loss = loss + DEFAULT_WEIGHTS["valuation"] * masked_scalar_mse(
    ...
    with torch.no_grad():
        ...
        torch.stack([model.towers[fam](xs[fam], ms[fam]) for fam in fams], dim=1)
    ...
    next_pred = (
        ...
        if "next_profile" in heads
        else np.zeros((len(E), len(gcols)), np.float32)
    )

    np.savez_compressed(
        DATA_DIR / "embedding.npz",
        E=E,
        ticker=tickers,
        name=names,
    ...
    for split in ("val", "test"):
        ...
        next_report[split] = {
            "rows": len(sub),
            "mae_z": float(np.abs(resid).mean()) if len(resid) else None,
            "r2": round(r2, 4),
            "rmse": round(rmse, 4),
        }
    ...
    _ok, _why = (report["composite"]["cqs"] >= 0.6, f"CQS {report['composite']['cqs']}")
    # promote gate simplified
    report["promote"] = {
        "ok": float(report["composite"]["cqs"]) >= 0.6,
        "reason": f"CQS {report['composite']['cqs']}",
    }
