...
def build_model(deeper=False):
    ...
    if deeper:
        ...
        model.entry_head = nn.Sequential(nn.Linear(args["dim"],64), nn.ReLU(), nn.Dropout(0.1), nn.Linear(64,32), nn.ReLU(), nn.Linear(32,1))
...
configs=[
    {"name":"bce_pw3.5_w4_gamma0", "pw":3.5, "gamma":0.0, "w":4.0, "lr":0.005, "epochs":15, "deeper":False},
    {"name":"bce_pw4.0_w4_gamma0", "pw":4.0, "gamma":0.0, "w":4.0, "lr":0.005, "epochs":15, "deeper":False},
    {"name":"bce_pw3.0_w5_gamma0_deep64", "pw":3.0, "gamma":0.0, "w":5.0, "lr":0.003, "epochs":18, "deeper":True},
    {"name":"bce_pw5.0_w5_gamma0", "pw":5.0, "gamma":0.0, "w":5.0, "lr":0.005, "epochs":12, "deeper":False},
]
...
for cfg in configs:
    ...
    for ep in range(cfg["epochs"]):
        ...
        print(f"Ep {ep+1}/{cfg['epochs']} loss {tot/max(1,nb):.4f} mean {mean:.3f} std {std:.3f} min {mn:.3f} max {mx:.3f} spread {mx-mn:.3f}")
        if mx>best_max:
            best_max=mx
            torch.save({"model":model.state_dict(), "cfg":cfg, "mean":mean, "max":mx}, f"pipeline/data/mtnn_entry_tuned_{cfg['name']}.pt")
