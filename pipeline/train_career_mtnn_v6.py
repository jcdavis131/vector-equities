...
ap.add_argument("--weight-entry", type=float, default=1.0)
ap.add_argument("--weight-vol", type=float, default=0.3)
ap.add_argument("--weight-dd", type=float, default=1.5)
ap.add_argument("--weight-nce", type=float, default=0.0)
            ...
            for s in range(0, len(val_seqs), Args.batch):
                ...
                for b in range(len(bl)):
                    for seq_pos in range(bl[b]["valid_len"]):
                        ...
                        if not np.isnan(fdd[b, seq_pos]):
                            pd_all.append(pd[b, seq_pos])
                            td_all.append(fdd[b, seq_pos])
