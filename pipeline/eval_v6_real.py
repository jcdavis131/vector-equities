...
ckpt=torch.load(DATA_DIR/"mtnn_career_v6_best.pt", map_location="cpu", weights_only=False)
args=ckpt["args"]
print(f"Loaded best epoch {ckpt.get('epoch')} IC {ckpt.get('ic')} prec20 {ckpt.get('prec20')} args {args}")
model=EquitiesCareerMTNN(fam_dims=fam_dims, d_tower=args.get("d_tower",24), d_tower_hidden=args.get("d_tower_hidden",64), d_emb=args.get("dim",64), d_time=8, d_time_emb=16, d_model=args.get("d_model",64), n_layers=args.get("n_layers",3), n_heads=args.get("n_heads",4), n_game=14, n_skills=0, n_sectors=11, n_archetypes=8, mlp_heads=True)
model.load_state_dict(ckpt["model"])
model.eval()

seqs, feat_to_idx, extra_idx = build_sequences(Z, mask, Z_raw, tickers, fiscal_years, sectors, manifest, fwd, max_seq_len=10)
...
def build_batch_one(seq):
    ...
    te_seq[0]=enc["time_enc"]; yn_seq[0]=enc["year_norm"]; vm[0]=enc["mask"]
...
pf_list=[]; tf_list=[]; pd_list=[]; td_list=[]; entry_list=[]; tick_list=[]
embeddings=[]
with torch.no_grad():
    for seq in latest_seqs:
        ...
        te_t=torch.tensor(te_seq); yn_t=torch.tensor(yn_seq); vm_t=torch.tensor(vm)
        ...
        pf_list.append(pf); tf_list.append(tf)
        pd_list.append(pd_); td_list.append(td)
        tick_list.append(seq["ticker"])
        emb = c_seq[0,pos,:].cpu().numpy()
        embeddings.append(emb)
        entry_list.append((seq["ticker"], pf, pd_, entry_prob, tf, td, triple))

pf_arr=np.array(pf_list); tf_arr=np.array(tf_list)
pd_arr=np.array(pd_list); td_arr=np.array(td_list)
...
scored = [(ticker, fwd_pred*entry_prob - dd_pred*0.2, fwd_pred, dd_pred, entry_prob, true_fwd, true_dd) for ticker,fwd_pred,dd_pred,entry_prob,true_fwd,true_dd,_ in entry_list]
scored_sorted=sorted(scored, key=lambda x: x[1], reverse=True)
print("Top 20 scored:")
for s in scored_sorted[:20]:
    print(s)

np.savez_compressed(DATA_DIR/"embedding_career_v6.npz", embedding=np.stack(embeddings), tickers=tick_list)
print("Saved embedding")

import pandas as pd
df=pd.DataFrame([{"ticker":t[0],"pred_fwd":t[1],"pred_dd":t[2],"entry_prob":t[3],"true_fwd":t[4],"true_dd":t[5],"triple":t[6],"score":t[1]*t[3]-t[2]*0.2} for t in entry_list])
