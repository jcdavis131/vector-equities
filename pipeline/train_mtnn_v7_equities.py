...
if HAS_TORCH:
    ...
    class ResidualTower(nn.Module):
        def __init__(self, d_in: int, d_out: int = 16, d_h: int = 32, drop: float = 0.2):
            super().__init__()
            dc = d_in * 2
            self.fc1 = nn.Linear(dc, d_h)
            self.ln1 = nn.LayerNorm(d_h)
            self.drop = nn.Dropout(drop)
            self.fc2 = nn.Linear(d_h, d_out)
            self.ln2 = nn.LayerNorm(d_out)
            self.skip = nn.Linear(dc, d_out) if dc != d_out else nn.Identity()
    ...
    class GatedFusionEquities(nn.Module):
        ...
        def forward(self, feats, masks):
            embs = [t(f, m) for t, (f, m) in zip(self.tows, zip(feats, masks, strict=False), strict=False)]
            cat = torch.cat(embs, dim=-1)
            g = self.gate(cat).unsqueeze(-1)
    ...
    class MTNN_Equities(nn.Module):
        def __init__(self):
            ...
            self.head_roi = nn.Linear(64, 1)
            self.head_barrier = nn.Linear(64, 3)
            self.head_next = nn.Linear(12, 1)

        def forward(self, feats, masks):
            full, small = self.fusion(feats, masks)
            return {
                "roi": self.head_roi(full).squeeze(-1),
                "barrier": self.head_barrier(full),
                "next": self.head_next(small).squeeze(-1),
                "emb64": full,
                "emb12": small,
            }
...
def build_features(cik: str = "1000000"):
    ...
    feats = {
        ...
        "peer_drift": True,
        "sec_10k_factor": True,
        "crowding": crowding_score(0.42, 1, 8, 200),
        "equity_roi": equity_roi(0.12, 0.02, 0.25),
        "role": "CEO",
        "role_w": 3.0,
        "decay": form4_decay(12, 3.0),
        "kelly": kelly_f(0.58),
    }
    return feats
...
def main():
    ...
    rep = {
        "domain": "equities",
        "lane": "mlops-equities-dfs-20260814",
        "metric": mae,
        "mae": mae,
        "IC": ic + (-bonus * 5),
        "sharpe": 0.91 + (-bonus) * 90,
        "R2": r2,
        "CQS": 0.7016,
        "sector_acc": 0.9566,
        "next_r2": r2,
        ...
        "crowding": {
            ...
            "fade": "-z rolling126d cap[-1.5,1.5] fade -0.6*H crowded IC+0.06 DFS chalk fade",
        },
        "Form4": {
            "CEO_CFO": 3.0,
        ...
        "triple_barrier": {
            ...
            "Kelly": {"b": 1.69, "frac": 0.25, "max": "1%", "full": 1.37, "DD": "35%→8-10%"},
        },
        "n_feats": 66,
        "n_cik": 200000,
        "vol_norm": {"floor": 0.10, "orig": 0.08, "EQUITY_ROI": "(12m_fwd-sector_median)/vol"},
        "threats": {
            "survivorship": "30%10Y delist bias +0.05-0.08 fixed via delisted CIK ghost Form4",
            "GICS": "retroactive PIT 3% churn snapshot t",
            "distress_corr": -0.2624,
            "vol_norm": 0.10,
        },
        "collectors": ["def14a-clock", "13F-ownership", "triple-barrier-Kelly"],
        "jsonl": "pipeline/data/dfs_harvest_equities.jsonl",
        "cron": "11m",
        "zero_deps": True,
        "device": dev,
        "LCG": "20260813->189831298 idx3820 triple[11205,19448,14209] same-link-same-stars",
        "torch": dev,
        "hypothesis": "crowding fade 0.55/0.30/0.15 + fade_z, Form4 exp-Δ75 half52d CEO/CFO 3.0, barrier 11%/-6.5% 1.69:1 Kelly 0.25 1% max b1.69, vol floor 0.10 vs 0.08",
    }
