                ...
                all_preds.append(
                    {
                        "ticker": t,
                        "fy": fy,
                        "entry": float(entry[b_idx, seq_pos]),
                        "fwd6": float(fwd_pred[b_idx, seq_pos]),
                        "dd": float(dd[b_idx, seq_pos]),
                        "fwd_vol": float(fwd_vol[b_idx, seq_pos]),
                        "sector": seq["sector"],
                    }
                )
...
md = [
    "# Career MTNN Trades - Relaxed (adaptive)",
    "",
    f"Model {ckpt_path} IC best {ckpt.get('ic')} prec {ckpt.get('prec20')}",
    f"Entry distrib mean {entries.mean():.3f} max {entries.max():.3f}",
    f"Strict 0.7/5%/-10% => {len(strict)} trades",
    "Relaxed 0.5/3%/-15% => {len(relax1)}",
    "Top20 fwd>2% dd>-20% sorted by entry*fwd6",
    "",
    "| Rank | Ticker | FY | Entry | Fwd6M | DD | Score | Sector |",
    "|---|---|---|---|---|---|---|---|",
]
for i, p in enumerate(top20, 1):
    md.append(
