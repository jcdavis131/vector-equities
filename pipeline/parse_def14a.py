            ...
            if m:
                ...
                try:
                    ...
                    if comp>=500000:  # threshold for NEO
                        neos.append({"name": first[:60], "title_guess": first, "total_comp_raw": clean_cells[-1], "total_comp": comp, "cells": clean_cells[:4]})
                except:
                    pass
    # Dedupe by name
    seen=set()
...
if __name__=="__main__":
    ...
    for hf in html_files[:args.limit*12]:  # approx
        ...
        parsed.append(res)
        print(f"{hf.name}: neos {res['neo_count']} success {res['parse_success']} board {res['board_size']} names {[n['name'][:20] for n in res['neos'][:3]]}")
    # Save master
    ...
    print(f"Wrote {out} {len(parsed)} records, success rate {sum(1 for p in parsed if p['parse_success'])}/{len(parsed)}")
