...
from feature_spec import ALL_FEATURES, FEATURE_FAMILIES, SECTORS, GAME_PROFILE_FEATURES
    ...
    for ticker, lst in by_ticker.items():
        ...
        for fy, ent in lst_sorted:
            ...
            for neo in neos_filtered:
                ...
                if "chief executive" in combined or " ceo" in combined or combined.strip().startswith("ceo") or "chief executive officer" in combined:
                    ceo_candidate=neo
                    break
            ...
            curr_neo_names=set([clean_name(n.get("name","")).lower() for n in neos_filtered if clean_name(n.get("name",""))])
            if prev_neo_names is None:
                ...
                ceo_change = (prev_ceo_name is not None and ceo_name.lower()!=prev_ceo_name)
...
def build_v5(limit=None):
    ...
    for entry in uni[:limit] if limit else uni:
        ...
        for yr in range(2015,2025):
            ...
            invested_cap=equity+debt-cash if (equity is not None and debt is not None and cash is not None) else None
            ...
            inst_pct=0.75; inst_delta=0.0; insider_net=0.0; float_pct=0.9; top10_conc=0.35; short_int=0.03
            ceo_age=55; ceo_tenure=6; ceo_founder=0; ceo_comp=12; ceo_eq=1.5; avg_neo=11; pay_ratio=200; board_indep=75; board_size=9; insider_own=3; ceo_pay_vs=0; neo_turn=0.15; ceo_dual=0
            ceo_change_flag=0
            ...
            if ef:
                ...
                sorted_ceo=sorted([ (fy_,nm) for fy_,nm in ceo_history_per_ticker[ticker] if fy_<=yr], key=lambda x: x[0])
            ...
            pe=None; pb=None; ps=None; ev_ebitda=None; ev_sales=None; earn_yield=None; fcf_yield=None
            ...
            rate_10y=rate_map.get(yr,3.0); vix_avg=vix_map.get(yr,16); credit_spread=3.5; gdp={2015:2.9,2016:1.8,2017:2.2,2018:2.9,2019:2.3,2020:-2.2,2021:5.8,2022:1.9,2023:2.5,2024:2.2}.get(yr,2)

            altman=None
            if assets and assets!=0 and equity and ret_earn is not None and ebit is not None:
                try:
                    wc=working_cap or 0; mv=equity; liab_v=liab or assets*0.6
            ...
            row_feat={
                ...
                "ROE":roe,"ROA":roa,"ROIC":roic,"FCF_ROIC":safe_div(fcf,invested_cap),"ROIC_WACC_SPREAD":(roic-0.08) if roic else None,
                ...
                "RET_1M":ret_1m,"RET_3M":ret_3m,"RET_6M":ret_6m,"RET_12M":ret_12m,"VOL_30D":vol_30d,"VOL_90D":vol_90d,"VOL_252D":vol_252d,"BETA_1Y":beta_1y,"VOLUME_AVG_30D":vol_avg_30d,"MOMENTUM_12_1":mom_12_1,
                "PE":pe,"PB":pb,"PS":ps,"EV_EBITDA":ev_ebitda,"EV_SALES":ev_sales,"EARNINGS_YIELD":earn_yield,"FCF_YIELD":fcf_yield,"DIV_YIELD":0.015,
                "NEO_COUNT":neo_count_real,"CEO_AGE":ceo_age,"CEO_TENURE":ceo_tenure,"CEO_FOUNDER_FLAG":ceo_founder,"CEO_TOTAL_COMP":ceo_comp,"CEO_EQUITY_PCT":ceo_eq,"AVG_NEO_COMP":avg_neo,"CEO_PAY_RATIO":pay_ratio,"BOARD_INDEP_PCT":board_indep,"BOARD_SIZE":board_size,"INSIDER_OWN_PCT":insider_own,"CEO_PAY_VS_SECTOR":ceo_pay_vs,"NEO_TURNOVER":neo_turn,"CEO_DUALITY":ceo_dual,
                ...
                "EARN_SURPRISE_STREAK":0,"GUIDANCE_RAISE_FLAG":0,"EPS_REVISION_UP_PCT":0.5,"PRICE_VS_52W_HIGH":price_vs_52w if price_vs_52w is not None else 0.9,"RSI_14_PROXY":rsi_14 if rsi_14 is not None else 50,"ACCIDENT_DISCLOSURE":0,
                "ALTMAN_Z":altman,"PIOTROSKI_F_SCORE_PROXY":5
            }
            for k in ALL_FEATURES:
                if k not in row_feat:
                    row_feat[k]=None
            prev_vals[f"REV_{yr}"]=rev; prev_vals[f"EBITDA_{yr}"]=ebitda; prev_vals[f"NET_{yr}"]=net; prev_vals[f"FCF_{yr}"]=fcf; prev_vals[f"EPS_{yr}"]=eps; prev_vals[f"BOOK_{yr}"]=book_value; prev_vals[f"OCF_{yr}"]=ocf; prev_vals[f"SHARES_{yr}"]=shares_d
            if rev is None and assets is None:
                continue
            all_rows.append({"ticker":ticker,"company":company,"sector":sector,"fiscal_year":str(yr),"features":row_feat, "price": price_at_fy})
            fwd_labels.append({
                "FWD_RET_1M": fwd_ret_1m,
                "FWD_RET_3M": fwd_ret_3m,
                "FWD_RET_6M": fwd_ret_6m,
    ...
    print(f"Collected {len(all_rows)} rows from {len(set(r['ticker'] for r in all_rows))} tickers (v5)")
    D=len(ALL_FEATURES); N=len(all_rows)
    Z_raw=np.zeros((N,D), dtype=np.float32); mask=np.zeros((N,D), dtype=np.float32)
    tickers=[]; names=[]; fyears=[]; sectors=[]; prices=[]
    for i,row in enumerate(all_rows):
        tickers.append(row["ticker"]); names.append(row["company"]); fyears.append(row["fiscal_year"]); sectors.append(row["sector"]); prices.append(row.get("price") or 0)
            ...
            if val is not None:
                try:
                    if isinstance(val,float) and (np.isnan(val) or np.isinf(val)):
                        continue
                    Z_raw[i,j]=float(val); mask[i,j]=1.0
                except:
                    pass
    ...
    for fy in sorted(set(fyears)):
        ...
        for j in range(D):
            col=Z_raw[rows,j]; m=mask[rows,j]; valid=col[m>0.5]
            if len(valid)==0:
                gvalid=Z_raw[:,j][mask[:,j]>0.5]; median=np.median(gvalid) if len(gvalid)>0 else 0.0
            else:
    ...
    for fy in sorted(set(fyears)):
        ...
        for j in range(D):
            ...
            if len(vals)<2:
                Z[rows,j]=0; continue
            mean=vals.mean(); std=max(vals.std(),1e-6)
            zs=(vals-mean)/std; zs=np.clip(zs,-4,4); Z[rows,j]=zs

    # forward labels arrays
    fwd_ret_1m = np.array([fl["FWD_RET_1M"] if fl["FWD_RET_1M"] is not None else np.nan for fl in fwd_labels], dtype=np.float32)
    fwd_ret_3m = np.array([fl["FWD_RET_3M"] if fl["FWD_RET_3M"] is not None else np.nan for fl in fwd_labels], dtype=np.float32)
    fwd_ret_6m = np.array([fl["FWD_RET_6M"] if fl["FWD_RET_6M"] is not None else np.nan for fl in fwd_labels], dtype=np.float32)
    fwd_ret_12m = np.array([fl["FWD_RET_12M"] if fl["FWD_RET_12M"] is not None else np.nan for fl in fwd_labels], dtype=np.float32)
    fwd_vol_6m = np.array([fl["FWD_VOL_6M"] if fl["FWD_VOL_6M"] is not None else np.nan for fl in fwd_labels], dtype=np.float32)
    fwd_dd_6m = np.array([fl["FWD_DD_6M"] if fl["FWD_DD_6M"] is not None else np.nan for fl in fwd_labels], dtype=np.float32)
    triple_barrier = np.array([fl["TRIPLE_BARRIER"] if fl["TRIPLE_BARRIER"] is not None else -1 for fl in fwd_labels], dtype=np.int64)
    ...
    total_rows=N
    exec_set_str=set((t,str(fy)) for t,fy in exec_features.keys())
    rows_with_neo_precise=sum(1 for t,fy in zip(tickers,fyears) if (t,fy) in exec_set_str)
    ...
    form4_keys_str=set((t,str(fy)) for t,fy in form4_flat.keys())
    rows_with_form4=sum(1 for t,fy in zip(tickers,fyears) if (t,fy) in form4_keys_str)
    ...
    for feat in ALL_FEATURES:
        ...
        if feat in ["RET_1M","RET_3M","RET_6M","RET_12M","VOL_30D","VOL_90D","VOL_252D","BETA_1Y","VOLUME_AVG_30D","MOMENTUM_12_1","PRICE_VS_52W_HIGH","RSI_14_PROXY"]:
            real_flags[feat]= rows_with_mhist/total_rows >0.5 if total_rows else False
        elif feat in ["PE","PB","PS","EV_EBITDA","EV_SALES","EARNINGS_YIELD","FCF_YIELD"]:
            real_flags[feat]= rows_with_mhist/total_rows >0.3
        elif feat in ["NEO_COUNT","CEO_TOTAL_COMP","AVG_NEO_COMP","CEO_TENURE","CEO_FOUNDER_FLAG","NEO_TURNOVER","BOARD_SIZE"]:
    ...
    manifest={
        ...
        "forward_labels": ["FWD_RET_1M","FWD_RET_3M","FWD_RET_6M","FWD_RET_12M","FWD_VOL_6M","FWD_DD_6M","TRIPLE_BARRIER","CEO_CHANGE_FLAG","TIME_SINCE_CEO_CHANGE"]
    }

    out_v5 = DATA_DIR / "train_matrix_v5.npz"
    np.savez_compressed(out_v5,
        Z=Z.astype(np.float32), mask=mask, ticker=np.array(tickers), name=np.array(names),
        fiscal_year=np.array(fyears), sector=np.array(sectors), cluster=np.zeros(N,dtype=np.int64),
        Z_raw=Z_raw,
        fwd_ret_1m=fwd_ret_1m, fwd_ret_3m=fwd_ret_3m, fwd_ret_6m=fwd_ret_6m, fwd_ret_12m=fwd_ret_12m,
        fwd_vol_6m=fwd_vol_6m, fwd_dd_6m=fwd_dd_6m, triple_barrier=triple_barrier,
        ceo_change_flag=ceo_change_flag, time_since_ceo=time_since_ceo,
        price=np.array(prices, dtype=np.float32)
    )
    # also alias as latest train_matrix
    out_alias = DATA_DIR / "train_matrix.npz"
    np.savez_compressed(out_alias, Z=Z.astype(np.float32), mask=mask, ticker=np.array(tickers), name=np.array(names), fiscal_year=np.array(fyears), sector=np.array(sectors), cluster=np.zeros(N,dtype=np.int64))
    ...
    print(f"Forward label coverage: 1M {np.isfinite(fwd_ret_1m).sum()}/{N} 3M {np.isfinite(fwd_ret_3m).sum()} 6M {np.isfinite(fwd_ret_6m).sum()} barrier {np.sum(triple_barrier!=-1)}")
