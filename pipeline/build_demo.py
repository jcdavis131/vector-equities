...
def gen_company_profile(n_companies=800, n_years=10, start_year=2015):
    ...
    for row in range(N):
        ...
        gross_margin = np.clip(np.random.normal(0.40 + qual*0.08 + arch_adj["margin"]*0.1, 0.18), 0.05, 0.95)
        op_margin = np.clip(gross_margin - np.random.normal(0.20,0.10), -0.5, 0.6)
        net_margin = np.clip(op_margin - np.random.normal(0.05,0.08) - max(0,lev_bias*0.02), -0.8, 0.5)
        ...
        debt_to_equity = np.clip(np.random.normal(0.6 + lev_bias*0.5 + arch_adj["debt"]*0.3, 0.6), 0.0, 3.5)
        debt_to_ebitda = np.clip(np.random.normal(2.0 + lev_bias*0.6, 1.5), 0.0, 8.0) if ebitda_margin>0 else np.random.normal(5,2)
        ...
        ev_ebitda = np.clip(np.random.normal(12 + grow_bias*2 + (1-roic)* -5, 6), 2, 40) if ebitda_margin>0 else np.random.normal(20,10)
