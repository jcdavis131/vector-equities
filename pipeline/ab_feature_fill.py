...
SC = Path(r"C:\Users\jcdav\AppData\Local\Temp\claude\C--Users-jcdav"
          r"\be69d382-ce38-4d23-b6d1-d92c62546c02\scratchpad\ab")
...
for arm, src in ARMS.items():
    for seed in SEEDS:
        ...
        cmd = [PY, "pipeline/train_mtnn.py", "--epochs", "60", "--dim", "64",
               "--fusion", "transformer", "--tower-blocks", "1", "--seed", str(seed)]
        ...
        print(f"  {arm:11} seed={seed:<3} recall={row['test_recall']:.3f} "
              f"purity={row['purity20']:.4f} sector={row['sector_acc']:.4f} "
              f"r2={row['next_r2_test']:.4f} CQS={row['cqs']:.4f}", flush=True)
