...
Loads assets/trades_final_ranked_v6.csv (233 trades, columns:
ticker,fy,entry,fwd6,dd,fwd_vol,sector,score,true_fwd,fwd6_before).
...
- triple_barrier_hit_rate: fraction where true_fwd >= +10% before -7% 63d (proxy: true_fwd >0.10 and
dd>-0.07 for csv; uses entry flag if present)
...
def compute():
    ...
    # distress early warning precision: among bottom decile dd<-0.15, distress head should flag — proxy via
    # correlation pred vs dd
    ...
    out = {
        ...
        "metrics": {
            "ic_rank_1m": {
                "score": round(ic_1m, 4) if math.isfinite(ic_1m) else None,
                "n": int(n_6m),
                "description": "Spearman pred 1M vs true 1M (scaled from 6M if univariate)",
            },
            "ic_rank_3m": {"score": round(ic_3m, 4) if math.isfinite(ic_3m) else None, "n": int(n_6m)},
            "ic_rank_6m": {
                "score": round(ic_6m, 4) if math.isfinite(ic_6m) else None,
                "n": int(n_6m),
                "description": "Spearman calibrated fwd6 vs true_fwd 12M realized; gate IC>0",
            },
            "ic_rank_12m": {"score": round(ic_12m, 4) if math.isfinite(ic_12m) else None, "n": int(n_6m)},
            "triple_barrier_hit_rate_10pct_before_minus7pct_63d": {
                "score": round(triple_rate, 4),
                "n": len(triple_hits),
                "definition": "+10% hit before -7% 63d (proxy true_fwd>=0.10 & dd>-0.07)",
            },
            "distress_early_warning_corr_pred_vs_dd": {
                "score": round(distress_corr, 4) if math.isfinite(distress_corr) else None,
                "description": "pred fwd6 vs dd threshold; positive means higher pred -> less distress",
            },
        ...
        "gate": {
            ...
            "note": "Gate promotion on IC>0 not just sector purity to prove embedding knows business future not just "
            "label",
        },
        "provenance": "Computed from published trades_final_ranked_v6.csv; isotonic calibration from "
        "assets/forward_calibration_isotonic.json",
    }
    return out
...
def main():
    ...
    print(
        f"Wrote {out_path} IC6M={out['metrics']['ic_rank_6m']['score']} "
        f"triple_rate={out['metrics']['triple_barrier_hit_rate_10pct_before_minus7pct_63d']['score']}"
    )
