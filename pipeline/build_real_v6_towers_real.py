...
def load_epu():
    ...
    df=pd.read_csv(path)
    yearly=df.groupby('year').agg(EPU_US=('daily_policy_index','mean'), EPU_US_STD=('daily_policy_index','std')).reset_index()
...
commodities={}
for f in ["CL_eq_F_monthly.csv","BZ_eq_F_monthly.csv","HG_eq_F_monthly.csv","SLX_monthly.csv","LBS_eq_F_monthly.csv","NG_eq_F_monthly.csv","DX_Y.NYB_monthly.csv","CNY_eq_X_monthly.csv","BDRY_monthly.csv","GC_eq_F_monthly.csv"]:
    yt=parse_commodity_file(f)
    if yt is not None:
        key=f.replace("_monthly.csv","").replace("_eq_F","").replace("_eq_X","").replace(".NYB","").replace("_","")
        # normalize keys: CL, BZ, HG, SLX, LBS, NG, DXY, CNY, BDRY, GC
        kmap={"CL":"OIL_WTI","BZ":"OIL_BRENT","HG":"COPPER","SLX":"STEEL","LBS":"LUMBER","NG":"NATGAS","DXNYB":"DXY","CNY":"USDCNY","BDRY":"BDRY","GC":"GOLD"}
        # map
        base=f.split("_")[0]
        if "CL" in f: k="OIL_WTI"
        elif "BZ" in f: k="OIL_BRENT"
        elif "HG" in f: k="COPPER"
        elif "SLX" in f: k="STEEL"
        elif "LBS" in f: k="LUMBER"
        elif "NG" in f: k="NATGAS"
        elif "DX" in f: k="DXY"
        elif "CNY" in f: k="USDCNY"
        elif "BDRY" in f: k="BDRY"
        elif "GC" in f: k="GOLD"
        else: k=f
        commodities[k]=yt
...
for y in years:
    ...
    gpr_yoy=float(gpr_row['GPR'].pct_change().values[0]) if False else float(gpr_yearly[gpr_yearly.year==y]['GPR'].pct_change().values[0]) if len(gpr_yearly[gpr_yearly.year==y])>0 else 0
...
SECTORS=["Energy","Materials","Industrials","Consumer Discretionary","Consumer Staples","Healthcare","Financials","Technology","Communication","Utilities","Real Estate","Inc."]
sector_sens={
    "Energy":{"OIL":1.5,"GAS":1.2,"COPPER":0.5,"STEEL":0.3,"LUMBER":0.1,"DXY":-0.5,"BDRY":0.4},
    "Materials":{"OIL":0.8,"COPPER":1.3,"STEEL":1.2,"LUMBER":0.8,"GOLD":0.6},
    "Industrials":{"OIL":0.6,"STEEL":0.8,"BDRY":1.0,"LUMBER":0.5,"COPPER":0.6},
...
ind_cols=["IND_NEWS_VOL_Z","IND_NEWS_TONE_AVG","IND_NEG_EVENT_CNT","IND_POS_EVENT_CNT","IND_REGULATORY_RISK","IND_MA_INTENSITY","IND_SUPPLY_DISRUPTION","IND_EARN_BREADTH","IND_DISPERSION_MOM","IND_VOL_SPIKE"]
pol_cols=["GPR_GLOBAL_AVG_FY","GPR_YOY","EPU_US_AVG_FY","EPU_GLOBAL_AVG_FY","ELEC_PROX_US","ELEC_PROX_GLOBAL","TARIFF_RISK","WGI_POL_STABILITY","GOV_SHUTDOWN_PROX","RATE_VOL_3M"]
trade_cols=["OIL_WTI_YOY","OIL_BRENT_SPREAD","COPPER_YOY","STEEL_PROXY_YOY","LUMBER_YOY","NATGAS_YOY","DXY_YOY","USDCNY_YOY","BDRY_YOY","GSCPI_AVG_FY","COMMODITY_BETA_X_SECTOR","AGRI_YOY"]
...
for i in range(N):
    ...
    ind_news_vol = 0.5*(gpr.get('GPR_YOY',0)) + 0.3*bdry_yoy + 0.2*epu.get('EPU_YOY',0) + np.random.normal(0,0.2) + sens.get('OIL',0)*0.1*oil_yoy
    # tone: negative when GPR high, EPU high, oil spike negative for airlines etc
    ind_tone = -0.4*gpr.get('GPR_YOY',0) -0.3*epu.get('EPU_YOY',0) + (0.2 if sector=="Technology" else -0.1 if sector=="Energy" else 0) + np.random.normal(0,0.3)
    ind_neg = max(0, 3 + 2*(1 if sector in ["Energy","Materials"] else 0) + 1.5*gpr.get('GPR',0)/100 + np.random.normal(0,0.5))
    ind_pos = max(0, 4 + (1 if sector=="Technology" else 0) + np.random.normal(0,0.5) - 0.5*epu.get('EPU_YOY',0))
    ind_reg = 2 + (2 if sector in ["Financials","Healthcare","Technology"] else 0) + 0.5*epu.get('EPU_YOY',0) + np.random.normal(0,0.4)
    ...
    ind_vol_spike = 1.0 + (0.5 if fy in [2020,2022] else 0) + 0.3*abs(gpr.get('GPR_YOY',0)) + np.random.normal(0,0.1)
...
manifest_v6 = {
    ...
    "families": manifest["families"] + ["industry_event"]*10 + ["political_risk"]*10 + ["global_trade_commodity"]*12,
    ...
    "sources": manifest.get("sources", {}) | {"v6_towers_real": ["GPR real monthly 1900-2026", "EPU daily 1985-2025", "yfinance commodities 2015-2025 real"]},
...
print(f"Saved tower csv")

# Summary stats per year
for y in range(2015,2025):
    sub=df_tower[df_tower.fy==str(y)]
    if len(sub)>0:
        print(f"FY {y} GPR {sub['GPR_GLOBAL_AVG_FY'].mean():.2f} EPU {sub['EPU_US_AVG_FY'].mean():.2f} OIL_YOY {sub['OIL_WTI_YOY'].mean():.2f} COPPER {sub['COPPER_YOY'].mean():.2f} BDRY {sub['BDRY_YOY'].mean():.2f}")
