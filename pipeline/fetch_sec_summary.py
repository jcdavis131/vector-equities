...
from fetch_sec import robust_fetch_json, CACHE, USER_AGENT

CACHE_SUM = CACHE / "sec_summary"
CACHE_SUM.mkdir(parents=True, exist_ok=True)

TAGS = {
 "REVENUE": ["Revenues","RevenueFromContractWithCustomerExcludingAssessedTax","SalesRevenueNet","SalesRevenueNetOrGrossProfit"],
 ...
 "CAPEX": ["PaymentsToAcquirePropertyPlantAndEquipment","CapitalExpenditures","PurchasesOfPropertyPlantAndEquipment"],
 "CURR_A": ["AssetsCurrent"],
 "CURR_L": ["LiabilitiesCurrent"],
 "DEPR": ["DepreciationDepletionAndAmortization","DepreciationAndAmortization","Depreciation"],
 "INTEREST": ["InterestExpense","InterestExpenseDebt"],
 "SHARES_D": ["WeightedAverageNumberOfDilutedSharesOutstanding","WeightedAverageNumberOfSharesOutstandingDiluted","CommonStockSharesOutstanding"],
...
def fetch_summary_for_cik(cik: str):
    ...
    summary["_meta"]={"cik":cik_pad,"entity":data.get("entityName"),"fetched":time.time()}
    # save
    try:
