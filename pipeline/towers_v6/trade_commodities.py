...
def _blocked_synthetic_trade_orig_dummy():years=range(2015, 2025)):  # noqa: B008 (immutable range default)
    ...
    rows = []
    for year in years:
        oil_yoy = np.random.normal(0.1, 0.4) + (-0.5 if year == 2020 else 0.6 if year == 2022 else 0)
        brent_spread = np.random.normal(2, 1)
        copper_yoy = np.random.normal(0.05, 0.3) + (0.4 if year == 2021 else -0.2 if year == 2020 else 0)
        steel_yoy = np.random.normal(0.08, 0.35)
        lumber_yoy = np.random.normal(0.0, 0.5) + (1.0 if year == 2021 else -0.6 if year == 2022 else 0)
        natgas_yoy = np.random.normal(0.2, 0.6) + (1.2 if year == 2022 else 0)
        dxy_yoy = np.random.normal(0.02, 0.08) + (0.1 if year == 2022 else -0.05 if year == 2023 else 0)
        usdcny_yoy = np.random.normal(0.02, 0.06) + (0.08 if year in [2018, 2019] else 0)
        bdry_yoy = np.random.normal(0.1, 0.8) + (-0.6 if year == 2020 else 1.0 if year == 2021 else 0)
        gscpi = np.random.normal(0, 1) + (2.5 if year in [2021, 2022] else -0.5 if year == 2023 else 0)
        commodity_beta = np.random.uniform(-0.5, 1.5)  # placeholder sector interaction later
