...
class ContinuousFusion(nn.Module):
    ...
    def __init__(self, n_towers: int, d_tower: int, d_time: int=8, d_time_emb: int=16, d_emb: int=48, d_hidden: int=192):
        super().__init__()
...
class CausalCareerTransformer(nn.Module):
    def __init__(self, d_emb: int=48, d_model: int=96, n_layers: int=4, n_heads: int=4, d_ff: int=256, dropout: float=0.1):
        super().__init__()
...
class EquitiesCareerMTNN(nn.Module):
    def __init__(self,
                 fam_dims: dict,
                 d_tower: int=24, d_tower_hidden: int=96,
                 d_emb: int=48,
                 d_time: int=8, d_time_emb: int=16,
                 d_model: int=96, n_layers: int=4, n_heads: int=4,
                 n_game: int=14, n_skills: int=12,
        ...
        self.towers = nn.ModuleDict({
            fam: ResidualTower(fam_dims[fam], d_out=d_tower, d_hidden=d_tower_hidden, n_blocks=n_tower_blocks)
            for fam in self.families
        })
        self.fusion = ContinuousFusion(len(self.families), d_tower, d_time=d_time, d_time_emb=d_time_emb, d_emb=d_emb)
        self.career_transformer = CausalCareerTransformer(d_emb=d_emb, d_model=d_model, n_layers=n_layers, n_heads=n_heads, dropout=dropout)
    ...
    def forward_sequence(self, xs_seq, ms_seq, time_enc_seq, year_norm_seq, mask_seq=None):
        ...
        """
        B, L = time_enc_seq.shape[0], time_enc_seq.shape[1]
        # encode each timestep independently then stack
        z_list=[]
        for l in range(L):
            xs_l = {fam: xs_seq[fam][:, l, :] for fam in self.families}
            ms_l = {fam: ms_seq[fam][:, l, :] for fam in self.families}
            t_l = time_enc_seq[:, l, :]
