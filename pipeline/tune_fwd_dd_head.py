...
ckpt=torch.load(DATA_DIR/"mtnn_career_best.pt", map_location="cpu", weights_only=False)
args=ckpt["args"]
...
for n,p in model.named_parameters():
    p.requires_grad = any(k in n for k in ["fwd_ret_head","fwd_dd_head","fwd_vol_head"])
trainable=sum(p.numel() for p in model.parameters() if p.requires_grad)
print(f"trainable fwd heads {trainable}")

opt=torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=0.003, weight_decay=1e-4)
...
def eval_stats():
    ...
    with torch.no_grad():
        for i in range(0,len(seqs),32):
            ...
            te_t=torch.tensor(te); yn_t=torch.tensor(yn)
            c_seq,z_seq,out=model.forward_sequence(xs_t,ms_t,te_t,yn_t,torch.tensor(vm))
            pred_f6=out["fwd_ret"][:,:,2].numpy()
            pred_dd=out["fwd_dd"].numpy()
            for b in range(len(bl)):
                for l in range(bl[b]["valid_len"]):
                    if np.isnan(f6[b,l]) or np.isnan(fd[b,l]): continue
                    pf.append(pred_f6[b,l])
                    tf.append(f6[b,l])
                    pd.append(pred_dd[b,l])
                    td.append(fd[b,l])
    pf=np.array(pf); tf=np.array(tf); pd=np.array(pd); td=np.array(td)
    # IC
    from scipy.stats import spearmanr
    ic_f6=spearmanr(pf,tf)[0]
    ic_dd=spearmanr(pd,td)[0]
    mean_f6=pf.mean(); std_f6=pf.std()
    mean_dd=pd.mean(); std_dd=pd.std()
    return mean_f6,std_f6,ic_f6, mean_dd,std_dd,ic_dd, pf,tf,pd,td

configs=[
    {"name":"fwd_w3_dd_w1_rank0.5_var0.1", "w_f6":3.0, "w_dd":1.0, "w_vol":0.2, "rank_w":0.5, "var_w":0.1, "lr":0.003, "epochs":12},
    {"name":"fwd_w5_dd_w1.5_rank1_var0.2", "w_f6":5.0, "w_dd":1.5, "w_vol":0.3, "rank_w":1.0, "var_w":0.2, "lr":0.002, "epochs":12},
]
...
for cfg in configs:
    ...
    for ep in range(cfg["epochs"]):
        ...
        for i in range(0,len(seqs),32):
            ...
            xs_t={fam: torch.tensor(xs[fam]) for fam in fams}
            ms_t={fam: torch.tensor(ms[fam]) for fam in fams}
            te_t=torch.tensor(te); yn_t=torch.tensor(yn); vm_t=torch.tensor(vm)
            ...
            mse_vol=F.mse_loss(pred_vol[mask_vol], t_vol[mask_vol]) if mask_vol.sum()>0 else torch.tensor(0.0)
        ...
        print(f"Ep {ep+1} loss {tot_loss/max(1,nb):.4f} f6 mean {mean_f6:.3f} std {std_f6:.3f} IC {ic_f6:.3f} | dd mean {mean_dd:.3f} std {std_dd:.3f} IC {ic_dd:.3f}")
        if ic_f6>best_ic:
            best_ic=ic_f6
            torch.save({"model":model.state_dict(), "cfg":cfg, "ic_f6":ic_f6, "stats":(mean_f6,std_f6,mean_dd,std_dd)}, f"pipeline/data/mtnn_fwd_tuned_{cfg['name']}.pt")
