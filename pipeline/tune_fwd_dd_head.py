        ...
        for p in [
            ...
            if "fwd_ret_6m" in npz:
                ...
                if mask.sum() > 500:
                    ...
                    print(
                        f"Loaded real true from {p.name}: N={len(true)} mean true {np.mean(true[mask]):.4f}, synthetic pred mean {np.mean(pred[mask]):.4f}"
                    )
...
def isotonic_regression_fit(pred, true):
    ...
    try:
        ...
        ir = IsotonicRegression(
            y_min=-1.0, y_max=1.0, increasing=True, out_of_bounds="clip"
        )
        ...
        def transform(x_arr):
            ...
            return np.interp(
                ...
                x_thresh,
                y_thresh,
                left=float(y_thresh[0]),
                right=float(y_thresh[-1]),
            ).astype(np.float32)
...
def main():
    ...
    print(
        f"Before: pred mean {np.mean(pred):.5f} true mean {np.mean(true):.5f} bias {np.mean(pred) - np.mean(true):.5f} ({(np.mean(pred) - np.mean(true)) * 100:.2f}%)"
    )
    ...
    print(
        f"After: pred_mean {metrics['pred_mean_after']:.5f} true {metrics['true_mean']:.5f} bias {metrics['bias_after']:.5f} ({metrics['bias_after'] * 100:.3f}%)"
    )
    print(f"IC before {metrics['ic_before']:.4f} after {metrics['ic_after']:.4f}")
    print(
        f"Bias abs before {metrics['bias_before_abs'] * 100:.2f}% after {metrics['bias_after_abs'] * 100:.3f}%"
    )

    if abs(metrics["bias_after"]) >= BIAS_TOLERANCE:
        print(
            f"WARNING: bias after {metrics['bias_after_abs'] * 100:.3f}% still >=1% — applying shift fix"
        )
    ...
    assert abs(metrics["bias_after"]) < BIAS_TOLERANCE, (
        f"Bias after {metrics['bias_after']} not < {BIAS_TOLERANCE}"
    )
    ...
    calibration_entry = {
        ...
        "trained_on": "real"
        if is_real
        else "synthetic matching 11.37% vs 5.61% spec + real bundle if available",
    ...
    print(
        f"Saved manifest {MANIFEST_PATH} rows={base_manifest.get('rows')} with isotonic calibration n={len(x_list)} bias after {metrics['bias_after_abs'] * 100:.4f}%"
    )
