...
from ingest_sec_v2 import CHUNKS_V2_DIR, DATA_DIR, load_universe, process_ticker
        ...
        if chk_path.exists() and not args.force:
            try:
                if chk_path.stat().st_size > 1000:
                    ...
                    if cnt > 0:
                        print(
                            f"[{idx + 1}/{len(subset)}] {ticker} SKIPPED cached chunks={cnt}"
                        )
                        # update existing
                        existing[ticker] = existing.get(
                            ticker,
                            {"ticker": ticker, "status": "ok", "chunk_count": cnt},
                        )
                        ok += 1
                        total_chunks += cnt
                        continue
    ...
    try:
        ...
        for jf in CHUNKS_V2_DIR.glob("*.json"):
            ...
            try:
                ...
                continue
        idx_path.write_text(
            json.dumps(sorted(idx_data, key=lambda x: x["ticker"]), indent=2)
        )
