...
def build_validation_report(embeddings, archetype_logits, clusters, sectors, fiscal_years, sector_labels, next_pred, game_target, next_idx, pairs, held_out_pairs=None):
    # simplified version
    ...
    for sec in sectors_unique[:5]: # top 5 for brevity
        ...
        slices[sec]={"rows":int(len(rows)),"confidence_mean":float(conf[rows].mean()) if len(rows) else None}
