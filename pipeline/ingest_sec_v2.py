...
def get_filings_by_type(sub_data: dict, target_forms: list[str], disallowed: list[str]) -> dict[str, list[dict]]:
    ...
    for i in range(min(len(forms), len(accessions))):
        ...
        if not acc or not pd:
            continue
        all_filings.append(
            {"form": f, "accessionNumber": acc, "primaryDocument": pd, "filingDate": fd, "reportDate": rd}
        )
...
def fetch_edgar_html(
    cik_nz: str, accession_nodash: str, primary_doc: str, dest_path: Path, delay: float = 0.3, force: bool = False
) -> tuple[bool, str]:
...
def parse_10k_html_v2(html_content: str, ticker_meta: dict) -> dict[str, list[dict]]:
    ...
    relevant_tags = body.find_all(
        ["p", "div", "table", "h1", "h2", "h3", "h4", "b", "strong", "li", "span", "font", "center"]
    )
    marker_indices = []
    ...
    def process_range(target_id, start_idx, end_idx):
        ...
        for ridx in range(start_idx + 1, end_idx):
            if ridx >= len(relevant_tags):  # noqa: F821 (closure over relevant_tags in parse_10k_html_v2)
                break
            tag = relevant_tags[ridx]  # noqa: F821 (closure over relevant_tags in parse_10k_html_v2)
            try:
                parent = tag.parent
                inside_table = False
                for _ in range(3):
            ...
            if is_subheader_candidate(txt_raw, tag.name):
                ...
                cur_buff.append({"type": "subheader", "text": txt_raw, "tag": tag.name})
                continue
            if cur_buff and cur_buff[-1].get("type") == "para" and cur_buff[-1].get("text") == txt_raw:
                continue
    ...
    if not boundaries and last_occurrence:
        ...
        for tgt in ["1", "1A", "7"]:
            if tgt in last_occurrence:
                ...
                for m_idx, _m_iid in sorted_last:
                    if m_idx > s:
                        next_idx = m_idx
                        break
...
def chunk_from_buffers(
    buffers: dict[str, list[dict]], ticker_meta: dict, filing_meta: dict, target_tokens: int = 750
) -> list[dict]:
    chunks = []
    ...
    for item_number in ["1", "1A", "7"]:
        ...
        for elem in elems:
            ...
            if et == "table":
                ...
                injected = (
                    f"[Table Context: {ticker} {form} Item {item_number} — "
                    f"{cur_header or elem.get('header') or 'Financial/Operational Table'}]\n{table_md}"
                )
...
def parse_def14a_v2(html_content: str, ticker_meta: dict, filing_meta: dict) -> list[dict]:
    ...
    for sec_name, pat in patterns.items():
        ...
        for p in paras:
            ...
            if len(cur) // 4 > 800:
                chunk = {
                    "id": f"{ticker_meta['ticker']}_{filing_meta['form']}_"
                    f"{sec_name}_{filing_meta.get('filing_year')}_{len(chunks)}",
...
def process_ticker(
    entry: dict, filing_limits: dict[str, int], delay: float = 0.3, force: bool = False, force_sub_fetch: bool = False
) -> dict:
    ticker = entry["ticker"]
    cik_pad = entry["cik"]
    cik_nz = entry["cik_nz"]
...
def main():
    ...
    if args.limit == 0:
        ...
        subset = universe[args.start : args.start + args.limit] if not args.ticker and not args.tickers else universe
    print(
        f"Processing {len(subset)} tickers from offset {args.start} limit {args.limit} "
        f"tickers={args.ticker or args.tickers or 'all'}"
    )
    ...
    for idx, entry in enumerate(subset):
        ...
        try:
            res = process_ticker(
                entry, filing_limits, delay=args.delay, force=args.force, force_sub_fetch=args.force_sub
            )
            print(
                f"  -> {res['status']} chunks={res.get('chunk_count',0)} biz={res.get('has_business')} "
                f"risk={res.get('has_risk')} mda={res.get('has_mda')}"
            )
        ...
        if (idx + 1) % 10 == 0:
            ...
            manifest_data = {
                ...
                "totals": {"ok": ok_cnt, "fail": fail_cnt, "total_chunks": total_chunks},
                "entries": all_entries,
            }
            try:
    ...
    manifest_data = {
        ...
        "totals": {"ok": ok_cnt, "fail": fail_cnt, "total_chunks": total_chunks, "total_entries": len(all_entries)},
        "entries": all_entries,
    }
    manifest_path.write_text(json.dumps(manifest_data, indent=2))
    print(
        f"\nDONE V2 ingest manifest {manifest_path} ok={ok_cnt} fail={fail_cnt} chunks={total_chunks} "
