...
for p in [
    SEC_SUBMISSIONS_DIR,
    SEC_10K_EXTRACTED_OLD,
    SEC_10K_EXTRACTED_V2,
    SEC_EDGAR_HTML,
    CHUNKS_V2_DIR,
]:
...
def find_universe_file() -> Path:
    candidates = [
        DATA_DIR / "universe_nasdaq_nyse_common_cik.json",
        DATA_DIR / "universe_full.json",
...
def load_universe(path: Path | None = None) -> list[dict]:
    ...
    def norm(e):
        ...
        if not ticker:
            return None
        cik_str = (
            re.sub(r"\D", "", str(cik)).zfill(10) if re.sub(r"\D", "", str(cik)) else ""
        )
        return {
            "ticker": ticker.upper(),
            "ticker_sec": e.get("ticker_sec") or ticker.upper(),
            "cik": cik_str,
            "cik_nz": str(int(cik_str)) if cik_str and cik_str.isdigit() else "",
            "company": e.get("company") or e.get("title") or ticker,
            "sector": e.get("sector") or "n/a",
            "industry": e.get("industry") or "n/a",
    ...
    for e in data:
        n = norm(e)
        if not n or not n["cik"] or n["ticker"] in seen:
            continue
        seen.add(n["ticker"])
        out.append(n)
    for e in supplement:
...
def robust_fetch_json_curl(url: str, timeout: int = 90) -> dict | None:
...
def fetch_submission_if_missing(cik_pad: str, delay: float = 0.25) -> dict | None:
...
def get_filings_by_type(
    sub_data: dict, target_forms: list[str], disallowed: list[str]
) -> dict[str, list[dict]]:
    filings = (
        sub_data.get("filings", {}) if isinstance(sub_data.get("filings"), dict) else {}
    )
    ...
    for i in range(min(len(forms), len(accessions))):
        ...
        if not f:
            continue
        f_up = f.upper().strip()
        if f_up in [d.upper() for d in disallowed] or f_up in ["3", "4", "5"]:
            continue
        if (
            f_up.startswith("13")
            or "13D" in f_up
            or "13G" in f_up
            or f_up in ["SC 13D", "SC 13G", "SC 13D/A", "SC 13G/A"]
        ):
            continue
        acc = accessions[i] if i < len(accessions) else ""
        pd = prim_docs[i] if i < len(prim_docs) else ""
        fd = filing_dates[i] if i < len(filing_dates) else ""
        rd = report_dates[i] if i < len(report_dates) else ""
        if not acc or not pd:
            continue
        all_filings.append(
            {
                "form": f,
    ...
    def sort_key(x):
        return x.get("filingDate") or "0000"
    ...
    for fil in all_filings_sorted:
        ft = fil["form"]
        if ft in grouped:
            grouped[ft].append(fil)
        if ft in ["DEF 14A", "DEF14A"]:
            grouped.setdefault("DEF14A", []).append(fil) if fil not in grouped.get(
                "DEF14A", []
            ) else None
    if "DEF 14A" in grouped and "DEF14A" in target_forms:
        for f in grouped["DEF 14A"]:
            if f not in grouped["DEF14A"]:
                grouped["DEF14A"].append(f)
    return grouped
...
def get_latest_filings(sub_data: dict, limit_per_type: dict[str, int]) -> list[dict]:
    ...
    for form, lim in limit_per_type.items():
        ...
        for c in candidates[:lim]:
            c_copy = dict(c)
            c_copy["requested_form"] = form
            out.append(c_copy)
    return out


def fetch_edgar_html(
    cik_nz: str,
    ...
    tmp_path = Path(
        f"/tmp/edgar_{cik_nz}_{accession_nodash}_{primary_doc.replace('/', '_')}.html"
    )
    try:
        if tmp_path.exists():
            tmp_path.unlink()
...
def is_boilerplate(text: str) -> bool:
    ...
    for ph in BOILERPLATE_PHRASES:
        if ph in tl:
            if len(tl) < 800:
                return True
            if sum(1 for kw in substance if kw in tl) < 2 and len(tl) < 1200:
                return True
    if len(tl) < 80 and tl.strip().endswith("..."):
        return True
    return False


ITEM_DETECTION_REGEX = re.compile(
    r"(?i)^\s*ITEM\s+([0-9]+[A-C]?)\b[^\n]{0,120}", re.MULTILINE
)


def detect_item_id(text: str) -> str | None:
    if len(text) > 300:
        snippet = text[:300]
    else:
        snippet = text
...
def is_subheader_candidate(text: str, tag_name: str) -> bool:
    ...
    if tag_name in ["h1", "h2", "h3", "h4", "b", "strong"]:
        if detect_item_id(txt):
            return False
        if len(txt) > 5:
            return True
    if txt.isupper() and len(txt.split()) <= 12 and len(txt) >= 8:
        return True
    ...
    try:
        ...
        for i, tr in enumerate(rows):
            ...
            if not cell_texts or all(not c for c in cell_texts):
                continue
            if len([c for c in cell_texts if c]) < 2:
                continue
...
def parse_10k_html_v2(html_content: str, ticker_meta: dict) -> dict[str, list[dict]]:
    ...
    for idx, tag in enumerate(relevant_tags):
        ...
        if iid:
            marker_indices.append((idx, iid))
    last_occurrence = {}
    for idx, iid in marker_indices:
        last_occurrence[iid] = idx
    sorted_last = sorted(
        [(idx, iid) for iid, idx in last_occurrence.items()], key=lambda x: x[0]
    )
    iid_to_sorted_pos = {iid: pos for pos, (idx, iid) in enumerate(sorted_last)}
    if not sorted_last:
        return {
            k: []
            for k in ["1", "1A", "1B", "1C", "2", "3", "5", "6", "7", "7A", "8", "9"]
        }
    boundaries = {}
    for target in ["1", "1A", "7"]:
        if target not in last_occurrence:
            continue
        start_idx = last_occurrence[target]
        cur_pos = iid_to_sorted_pos.get(target, -1)
        if cur_pos == -1:
            continue
        if cur_pos + 1 < len(sorted_last):
            end_idx = sorted_last[cur_pos + 1][0]
        else:
            end_idx = min(len(relevant_tags), start_idx + 8000)
        boundaries[target] = (start_idx, end_idx)
    buffers = {
        k: [] for k in ["1", "1A", "1B", "1C", "2", "3", "5", "6", "7", "7A", "8", "9"]
    }
    ...
    def process_range(target_id, start_idx, end_idx):
        ...
        for ridx in range(start_idx + 1, end_idx):
            ...
            try:
                ...
                if inside_table and tag.name != "table":
                    continue
            except:
                pass
            if tag.name == "table":
                    ...
                    cur_buff.append(
                        {
                            "type": "table",
                            "text": md,
                            "header": last_subheader,
                            "tag": "table",
                        }
                    )
                continue
            try:
                txt_raw = tag.get_text(separator=" ", strip=True)
            except:
                continue
            if not txt_raw or len(txt_raw) < 30:
                if (
                    len(txt_raw) >= 3
                    and len(txt_raw) <= 200
                    and is_subheader_candidate(txt_raw, tag.name)
                ):
                    last_subheader = txt_raw
                    cur_buff.append(
                        {"type": "subheader", "text": txt_raw, "tag": tag.name}
                    )
                continue
            if len(txt_raw) < 350:
                iid = detect_item_id(txt_raw)
                if iid and iid != target_id:
                    break
            if is_boilerplate(txt_raw):
                continue
            ...
            if (
                cur_buff
                and cur_buff[-1].get("type") == "para"
                and cur_buff[-1].get("text") == txt_raw
            ):
                continue
            cur_buff.append(
                {
            ...
            if len(cur_buff) > 2500:
                break

    for tgt, (s, e) in boundaries.items():
        process_range(tgt, s, e)
...
def chunk_from_buffers(
    ...
    year = filing_meta.get("filing_year") or "2024"
    quarter = filing_meta.get("filing_quarter")
    cik = ticker_meta.get("cik") or ""
    company = ticker_meta.get("company") or ticker
    acc = filing_meta.get("accessionNumber") or ""
    fdate = filing_meta.get("filingDate") or ""
    surl = filing_meta.get("source_url") or ""
    global_idx = 0
    for item_number in ["1", "1A", "7"]:
        ...
        for elem in elems:
            ...
            if et == "table":
                if cur_text.strip() and len(cur_text) > 200:
                    ...
                    chunk = {
                        ...
                        "ticker": ticker,
                        "filing_type": form,
                        "filing_year": int(year) if str(year).isdigit() else year,
                        "filing_quarter": quarter or ("FY" if form == "10-K" else None),
                        "item_number": item_number,
                        "section_header": cur_header
                        or elem.get("header")
                        or last_header,
                        "text": cur_text.strip(),
                        "token_count": len(cur_text) // 4,
                        "source_url": surl,
                        "accessionNumber": acc,
                        "filingDate": fdate,
                        "cik": cik,
                    ...
                    if not is_boilerplate(chunk["text"]) and len(chunk["text"]) > 200:
                        ...
                        global_idx += 1
                    cur_text = ""
                    cur_header = last_header or cur_header
                ...
                chunk = {
                    ...
                    "filing_quarter": quarter or ("FY" if form == "10-K" else None),
                    "item_number": item_number,
                    "section_header": cur_header or elem.get("header") or last_header,
                    "text": injected,
                    "token_count": len(injected) // 4,
                    "source_url": surl,
                    "accessionNumber": acc,
                    "filingDate": fdate,
                    "cik": cik,
            ...
            if et == "para":
                ...
                if not cur_header:
                    cur_header = elem.get("header") or last_header or ""
                if cur_text:
                    cur_text += "\n\n"
                cur_text += txt
                if len(cur_text) // 4 >= target_tokens:
                    ...
                    chunk = {
                        ...
                        "filing_quarter": quarter or ("FY" if form == "10-K" else None),
                        "item_number": item_number,
                        "section_header": cur_header or last_header,
                        "text": cur_text.strip(),
                        "token_count": len(cur_text) // 4,
                        "source_url": surl,
                        "accessionNumber": acc,
        ...
        if cur_text.strip():
            if (
                chunks
                and chunks[-1]["item_number"] == item_number
                and not chunks[-1].get("has_table")
                and len(cur_text) // 4 < 300
            ):
...
def parse_def14a_v2(
    html_content: str, ticker_meta: dict, filing_meta: dict
) -> list[dict]:
    if not HAS_BS4:
        return []
    soup = BeautifulSoup(html_content, "lxml")
    ...
    patterns = {
        "BOARD": r"(?i)(CORPORATE GOVERNANCE|BOARD OF DIRECTORS|DIRECTOR INDEPENDENCE)",
        "CDA": r"(?i)(COMPENSATION DISCUSSION AND ANALYSIS)",
        "EXEC_COMP": r"(?i)(EXECUTIVE COMPENSATION|SUMMARY COMPENSATION TABLE)",
    }
    chunks = []
    for sec_name, pat in patterns.items():
        ...
        for p in paras:
            ...
            if len(cur) // 4 > 800:
                chunk = {
                    "id": f"{ticker_meta['ticker']}_{filing_meta['form']}_{sec_name}_{filing_meta.get('filing_year')}_{len(chunks)}",
                    "company_ticker": ticker_meta["ticker"],
                    "ticker": ticker_meta["ticker"],
                    "filing_type": "DEF14A",
                    "filing_year": filing_meta.get("filing_year") or 2024,
                    "filing_quarter": None,
                    "item_number": sec_name,
                    "section_header": sec_name,
                    "text": cur.strip(),
                    "token_count": len(cur) // 4,
                    ...
                    "company": ticker_meta.get("company", ""),
                    "chunk_index": len(chunks),
                    "has_table": False,
                    "metadata": {
                        "company_ticker": ticker_meta["ticker"],
                        "filing_type": "DEF14A",
                        "filing_year": filing_meta.get("filing_year"),
                        "filing_quarter": None,
                        "item_number": sec_name,
                        "section_header": sec_name,
                    },
...
def parse_10q_item2_v2(
    ...
    for p in paras:
        ...
        if len(cur) // 4 > 800:
            chunk = {
                "id": f"{ticker_meta['ticker']}_10-Q_2_{filing_meta.get('filing_year')}_{len(chunks)}",
                "company_ticker": ticker_meta["ticker"],
                "ticker": ticker_meta["ticker"],
                "filing_type": "10-Q",
                "filing_year": filing_meta.get("filing_year") or 2024,
                "filing_quarter": filing_meta.get("filing_quarter") or "Q",
                "item_number": "2",
                "section_header": cur_header,
                "text": cur.strip(),
                "token_count": len(cur) // 4,
                ...
                "metadata": {
                    ...
                    "filing_type": "10-Q",
                    "filing_year": filing_meta.get("filing_year"),
                    "filing_quarter": filing_meta.get("filing_quarter"),
                    "item_number": "2",
                    "section_header": cur_header,
                },
            }
...
def parse_8k_ex99_v2(
    ...
    for p in paras:
        ...
        if len(cur) // 4 > 800:
            chunk = {
                "id": f"{ticker_meta['ticker']}_8-K_EX99_{filing_meta.get('filing_year')}_{len(chunks)}",
                "company_ticker": ticker_meta["ticker"],
                "ticker": ticker_meta["ticker"],
                "filing_type": "8-K",
                "filing_year": filing_meta.get("filing_year") or 2024,
                "filing_quarter": None,
                "item_number": "EX99",
                "section_header": "Exhibit 99.1 Press Release",
                "text": cur.strip(),
                "token_count": len(cur) // 4,
                "source_url": filing_meta.get("source_url", ""),
                "accessionNumber": filing_meta.get("accessionNumber", ""),
...
def derive_filing_year_quarter(report_date: str, filing_date: str, form: str):
    ...
    if report_date and len(report_date) >= 4:
        year = report_date[:4]
    elif filing_date and len(filing_date) >= 4:
        year = filing_date[:4]
    quarter = None
    if form == "10-K":
        quarter = "FY"
...
def process_ticker(
    entry: dict,
    filing_limits: dict[str, int],
    delay: float = 0.3,
    force: bool = False,
    force_sub_fetch: bool = False,
) -> dict:
    ticker = entry["ticker"]
    cik_pad = entry["cik"]
    cik_nz = entry["cik_nz"]
    ...
    for filing in filings_to_fetch:
        form = filing.get("requested_form") or filing.get("form")
        acc = filing["accessionNumber"]
        acc_nodash = acc.replace("-", "")
        primary_doc = filing["primaryDocument"]
        filing_date = filing.get("filingDate", "")
        report_date = filing.get("reportDate", "")
        filing_year, filing_quarter = derive_filing_year_quarter(
            report_date, filing_date, form
        )
        source_url = f"https://www.sec.gov/Archives/edgar/data/{cik_nz}/{acc_nodash}/{primary_doc}"
        html_dir = SEC_EDGAR_HTML / cik_pad
        html_path = html_dir / f"{acc_nodash}_{primary_doc.replace('/', '_')}.html"
        ok, code = fetch_edgar_html(
            cik_nz, acc_nodash, primary_doc, html_path, delay=delay, force=force
        )
        fetched_html_status.append(
            {
                "form": form,
                "accession": acc,
        ...
        try:
            if form == "10-K":
                ...
                chunks = chunk_from_buffers(
                    buffers, ticker_meta, filing_meta, target_tokens=750
                )
                all_chunks.extend(chunks)
                for ch in chunks:
                    if ch["item_number"] == "1":
                        business_text_accum += "\n\n" + ch["text"]
                    elif ch["item_number"] == "1A":
                        risk_text_accum += "\n\n" + ch["text"]
                    elif ch["item_number"] == "7":
                        mda_text_accum += "\n\n" + ch["text"]
            elif form in ["DEF14A", "DEF 14A"]:
                all_chunks.extend(
                    parse_def14a_v2(html_content, ticker_meta, filing_meta)
                )
            elif form == "10-Q":
                all_chunks.extend(
                    parse_10q_item2_v2(html_content, ticker_meta, filing_meta)
                )
            elif form == "8-K":
                all_chunks.extend(
                    parse_8k_ex99_v2(html_content, ticker_meta, filing_meta)
                )
            else:
                buffers = parse_10k_html_v2(html_content, ticker_meta)
                all_chunks.extend(
    ...
    try:
        ...
        print(f"Failed to write chunks for {ticker}: {e}")
    if business_text_accum:
        (SEC_10K_EXTRACTED_V2 / f"{ticker}_business.txt").write_text(
            business_text_accum[:20000], encoding="utf-8"
        )
        (SEC_10K_EXTRACTED_OLD / f"{ticker}_business.txt").write_text(
            business_text_accum[:20000], encoding="utf-8"
        )
    if risk_text_accum:
        (SEC_10K_EXTRACTED_V2 / f"{ticker}_risk.txt").write_text(
            risk_text_accum[:20000], encoding="utf-8"
        )
        (SEC_10K_EXTRACTED_OLD / f"{ticker}_risk.txt").write_text(
            risk_text_accum[:20000], encoding="utf-8"
        )
    if mda_text_accum:
        (SEC_10K_EXTRACTED_V2 / f"{ticker}_mda.txt").write_text(
            mda_text_accum[:20000], encoding="utf-8"
        )
        (SEC_10K_EXTRACTED_OLD / f"{ticker}_mda.txt").write_text(
            mda_text_accum[:20000], encoding="utf-8"
        )
    try:
    ...
    cnt = 0
    try:
        cnt = len(json.loads(out_path.read_text()))
    except:
        pass
    ...
    return {
        ...
        "status": status,
        "chunk_count": cnt,
        "has_business": (SEC_10K_EXTRACTED_V2 / f"{ticker}_business.txt").exists(),
        "has_risk": (SEC_10K_EXTRACTED_V2 / f"{ticker}_risk.txt").exists(),
        "has_mda": (SEC_10K_EXTRACTED_V2 / f"{ticker}_mda.txt").exists(),
        "chunks_file": str(out_path),
        "filings_fetched": fetched_html_status,
    }
...
def main():
    ...
    filing_types_requested = [
        ft.strip().upper().replace(" ", "")
        for ft in args.filing_types.split(",")
        if ft.strip()
    ]
    normalized = []
    for ft in filing_types_requested:
        if ft in ["DEF14A", "DEF-14A", "DEF14A", "DEF 14A"]:
            normalized.append("DEF14A")
        else:
    ...
    for ft in filing_types_requested:
        if ft == "10-K":
            ...
            filing_limits[ft] = 1
    print(
        f"Filing limits (rank-ordered): {filing_limits} (Do NOT ingest 3,4,5,13D/G per guide)"
    )
    if args.ticker:
        universe = [u for u in universe if u["ticker"] == args.ticker.upper()]
    elif args.tickers:
        ...
        if args.limit and args.limit > 0 and args.limit < len(universe):
            ...
            big_set = set(big5)
            for u in universe:
                if u["ticker"] in big_set:
                    front.append(u)
            for u in universe:
                if u["ticker"] not in big_set:
                    rest.append(u)
            front_sorted = sorted(
                front,
                key=lambda x: big5.index(x["ticker"]) if x["ticker"] in big5 else 99,
            )
            universe = front_sorted + rest
    if args.limit == 0:
        subset = universe[args.start :]
    else:
        subset = (
            universe[args.start : args.start + args.limit]
            if not args.ticker and not args.tickers
            else universe
        )
    print(
        f"Processing {len(subset)} tickers from offset {args.start} limit {args.limit} tickers={args.ticker or args.tickers or 'all'}"
    )
    manifest_path = (
        Path(args.manifest) if args.manifest else DATA_DIR / "sec_v2_manifest.json"
    )
    existing_manifest = {}
        ...
        try:
            ...
            if isinstance(data, dict) and "entries" in data:
                for e in data["entries"]:
                    existing_manifest[e.get("ticker")] = e
        except:
            pass
    new_entries = []
    ok_cnt = 0
    ...
    for idx, entry in enumerate(subset):
        ...
        try:
            res = process_ticker(
                ...
                delay=args.delay,
                force=args.force,
                force_sub_fetch=args.force_sub,
            )
            print(
                f"  -> {res['status']} chunks={res.get('chunk_count', 0)} biz={res.get('has_business')} risk={res.get('has_risk')} mda={res.get('has_mda')}"
            )
        ...
        if (idx + 1) % 10 == 0:
            ...
            try:
                ...
                print(
                    f"  Manifest interim written {manifest_path} ok={ok_cnt} fail={fail_cnt} chunks={total_chunks}"
                )
            except Exception as e:
                print(f"Failed manifest write {e}")
    merged = {**existing_manifest}
    for e in new_entries:
        merged[e["ticker"]] = e
    all_entries = list(merged.values())
    ...
    try:
                ...
                index_data.append(
                    {
                        "ticker": jf.stem,
                        "chunk_count": len(ch),
                        "has_business": any(c.get("item_number") == "1" for c in ch),
                        "has_risk": any(c.get("item_number") == "1A" for c in ch),
                        "has_mda": any(c.get("item_number") == "7" for c in ch),
                        "has_table": any(c.get("has_table") for c in ch),
                    }
                )
            except:
                continue
        ...
        print(
            f"Chunks index written {chunks_index_path} with {len(index_data_sorted)} tickers"
        )
    except Exception as e:
        print(f"Failed chunks index {e}")
