...
def parse_xbrl_names(raw_html):
    ...
    try:
        pat = re.compile(
            r'<(?:ix:nonNumeric|xbrli:nonNumeric|span)[^>]*name=["\'](?:[^"\']*?)(?:PeoName|NameOfExecutiveOfficer|ExecutiveOfficerName|NameOf.*Officer)[^"\']*["\'][^>]*>([^<]{3,80})</[^>]+>',
            re.I,
        )
        for m in pat.finditer(raw_html):
            ...
            if is_plausible_name(cn):
                found.append(cn)
        pat2 = re.compile(
            r'dimension=["\']ecd:ExecutiveNameAxis["\'][^>]*>.*?<[^>]*\.domain[^>]*>([^<]{3,80})</[^>]*>',
            re.I | re.DOTALL,
        )
        for m in pat2.finditer(raw_html):
            ...
            if is_plausible_name(cn):
                found.append(cn)
        pat3 = re.compile(
            r"<ecd:ExecutiveNameAxis\.domain[^>]*>([^<]{3,80})</ecd:ExecutiveNameAxis\.domain>",
            re.I,
        )
        ...
        final.append(
            {
                "name": name,
                "role": n.get("role"),
                "total_comp": n.get("total_comp"),
                "comp_raw": n.get("comp_raw"),
            }
        )
...
if __name__ == "__main__":
    ...
    for i, f in enumerate(files, 1):
        try:
            ...
            r = {
                ...
                "filing_date": f.name.split("_")[1] if "_" in f.name else "",
                "file": f"pipeline/cache/sec_def14a/{f.name}",
                "neo_count": 0,
                "neos": [],
                "board_size": None,
                "candidates_found": 0,
    ...
    try:
        ...
        total = len(all_lines)
        print(
            f"\nWrote {out} total={total} succ={succ} rate={succ / total:.3f}",
            flush=True,
        )
