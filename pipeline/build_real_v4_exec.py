...
def load_def14a():
    ...
    print(f"Computed exec features for {len(exec_features)} (ticker,FY) combos, tickers {len(set(k[0] for k in exec_features))}")
...
def build_v4(limit=None):
    ...
    for entry in uni[:limit] if limit else uni:
        ...
        for yr in range(2015,2025):
            ...
            if mkt:
                ret_1m = 0.0; ret_3m = 0.0; ret_6m = 0.0; vol_30d = 0.0; vol_90d = 0.0; beta_1y = 0.0; mom_12_1 = 0.0
            else:
                ret_1m = None; ret_3m = None; ret_6m = None; vol_30d = None; vol_90d = None; beta_1y = None; mom_12_1 = None
            ...
            row_feat={
                ...
                "PE":None,"PB":None,"PS":None,"EV_EBITDA":None,"EV_SALES":None,"EARNINGS_YIELD":None,"FCF_YIELD":None,"DIV_YIELD":0.015,
                ...
                "EARN_SURPRISE_STREAK":0,"GUIDANCE_RAISE_FLAG":0,"EPS_REVISION_UP_PCT":0.5,"PRICE_VS_52W_HIGH":price_vs_52w if price_vs_52w is not None else 0.9,"RSI_14_PROXY":50,"ACCIDENT_DISCLOSURE":0,
            ...
            if rev is None and assets is None:
                continue
            all_rows.append({"ticker":ticker,"company":company,"sector":sector,"fiscal_year":str(yr),"features":row_feat})

    print(f"Collected {len(all_rows)} rows from {len(set(r['ticker'] for r in all_rows))} tickers (v4)")
    ...
    for i,row in enumerate(all_rows):
        tickers.append(row["ticker"]); names.append(row["company"]); fyears.append(row["fiscal_year"]); sectors.append(row["sector"])
    ...
    rows_with_neo = sum(1 for i in range(N) if (tickers[i], int(fyears[i]) if fyears[i].isdigit() else fyears[i]) in exec_features or (tickers[i], fyears[i]) in [(k[0], str(k[1])) for k in neo_keys])
    ...
    tickers_with_neo = len(set(t for t,fy in exec_set_str))
    # market coverage
    ...
    tickers_with_form4 = len(set(t for t,_ in form4_flat.keys()) & set(tickers))
    ...
    for feat in ["NEO_COUNT","CEO_TOTAL_COMP","AVG_NEO_COMP","CEO_TENURE","CEO_FOUNDER_FLAG","NEO_TURNOVER","BOARD_SIZE"]:
    ...
    np.savez_compressed(out_v4, Z=Z.astype(np.float32), mask=mask, ticker=np.array(tickers), name=np.array(names), fiscal_year=np.array(fyears), sector=np.array(sectors), cluster=np.zeros(N,dtype=np.int64), Z_raw=Z_raw)
    np.savez_compressed(out_real, Z=Z.astype(np.float32), mask=mask, ticker=np.array(tickers), name=np.array(names), fiscal_year=np.array(fyears), sector=np.array(sectors), cluster=np.zeros(N,dtype=np.int64), Z_raw=Z_raw)
    ...
    if not embed_path.exists():
        ...
        np.savez_compressed(embed_path, embedding=np.random.randn(N,32).astype(np.float32), ticker=np.array(tickers), fiscal_year=np.array(fyears))
    ...
    print(f"Coverage: NEO {rows_with_neo_precise}/{N} {rows_with_neo_precise/N*100:.1f}%, Market {rows_with_market}/{N} {rows_with_market/N*100:.1f}%, Form4 {rows_with_form4}/{N} {rows_with_form4/N*100:.1f}%")
    return out_v4
