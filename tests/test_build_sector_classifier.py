...
def load_module():
    mod_path = pathlib.Path("/home/hatch/workspace/vector-equities/pipeline/build_sector_classifier.py")
...
def test_smoke():
    mod = load_module()
    for fn in ["normalize_sector","load_universe","get_chunk_text","build_training_data","train_classifier","save_model"]:
...
def test_normalize_sector_various():
    ...
    assert mod.normalize_sector("N/A") == "Misc" or mod.normalize_sector("n/a")=="Misc"
    assert mod.normalize_sector("Basic Industries") in ("Materials","Industrials","Materials")  # mapped
