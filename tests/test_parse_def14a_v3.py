...
def load_module():
    mod_path = pathlib.Path("/home/hatch/workspace/vector-equities/pipeline/parse_def14a_v3.py")
...
def test_smoke():
    mod = load_module()
    for fn in ["clean_text","clean_name","is_blacklisted","is_plausible_name","extract_comp","score_table_text","parse_one_file_fast"]:
...
def test_clean_name():
    mod = load_module()
    assert mod.clean_name("  TIM COOK  ") == "Tim Cook" or "Tim" in mod.clean_name("  TIM COOK  ")
    assert mod.clean_name("") == "" or mod.clean_name("") is not None
    # numeric noise
    assert mod.clean_name("1234") == "" or mod.clean_name("1234") == "1234"  # plausible filter may reject
...
def test_parse_one_file_fast(tmp_path):
    ...
    p.write_text("<html><table><tr><td>Tim Cook</td><td>CEO</td><td>$15,000,000</td></tr></table> Summary Compensation Table</html>")
