...
def load_module():
    mod_path = pathlib.Path(
        "/home/hatch/workspace/vector-equities/pipeline/build_exec_registry_v3.py"
    )
...
def test_normalize_firstlast():
    ...
    assert (
        mod.normalize_firstlast("Madonna") == ("madonna", "")
        or len(mod.normalize_firstlast("Madonna")) == 2
    )
