...
def load_module():
    mod_path = pathlib.Path("/home/hatch/workspace/vector-equities/pipeline/defunct_companies.py")
...
def test_smoke_funcs():
    mod = load_module()
    for fn in ["is_known_defunct","get_defunct_info","parse_date","is_stale_trading","is_defunct_ticker","classify_status","list_defunct_universe","detect_via_sec_filings"]:
...
def test_classify_status_fields():
    mod = load_module()
    res = mod.classify_status("LEH", last_close_date_str="2008-09-12", first_close_date_str="1970-01-01")
