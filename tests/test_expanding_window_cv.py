...
def load_module():
    mod_path = pathlib.Path("/home/hatch/workspace/vector-equities/pipeline/expanding_window_cv.py")
...
def test_get_folds_basic():
    mod = load_module()
    cv = mod.ExpandingWindowCV(years=list(range(2015,2025)), min_train_years=5, val_years=1, step=1)
    folds = cv.get_folds()
    assert len(folds) > 0
...
def test_split_sample_years():
    ...
    cv = mod.ExpandingWindowCV(years=range(2015,2023), min_train_years=3, val_years=1, step=1)
...
def test_temporal_stability():
    mod = load_module()
    cv = mod.ExpandingWindowCV(years=range(2015,2025), min_train_years=5, val_years=1, step=1)
    folds = cv.get_folds()
...
def test_expanding_window_indices_helper():
    mod = load_module()
    splits = mod.expanding_window_indices(sample_years=list(range(2015,2025))*2, min_train_years=5, val_years=1, step=2)
    assert len(splits) >0
