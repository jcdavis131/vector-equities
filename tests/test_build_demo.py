...
try:
    from pipeline import build_demo as target_module
except ImportError:
    try:
...
@pytest.fixture
def sample_data():
    return {"module": "build_demo", "input": 1}


@pytest.mark.parametrize("input_val,expected", [(1, 2), (None, None), (0, 0)])
def test_build_demo_basic(input_val, expected, tmp_path):
    """Basic functionality smoke test - currently unimplemented (gap)."""
    if target_module is None:
        pytest.skip(f"pipeline.build_demo not importable")
...
def test_build_demo_invalid_inputs(bad_input, tmp_path):
    if target_module is None:
        pytest.skip(f"pipeline.build_demo not importable")
    pytest.skip("TODO: implement invalid-input handling")


def test_build_demo_integration(sample_data, tmp_path):
    tmp_file = tmp_path / f"build_demo_sample.json"
