...
def load_module():
    mod_path = pathlib.Path("/home/hatch/workspace/vector-equities/pipeline/build_chunks_v3_clean.py")
...
def test_smoke():
    mod = load_module()
    for fn in ["normalize_sector","tokenize","shingles","jaccard","dedup_paragraphs","clean_paragraph","is_boilerplate","split_into_chunks"]:
...
def test_dedup_paragraphs():
    mod = load_module()
    paras = ["This is a unique paragraph about Apple Inc business model.",
...
def test_process_ticker_no_text(monkeypatch):
    ...
    res = mod.process_ticker({"ticker":"FAKE","sector":"Technology","company":"Fake Co"})
...
def test_process_ticker_with_text(tmp_path, monkeypatch):
    ...
    long_text = ("Business overview of company selling tech gadgets and providing cloud infrastructure services to enterprise customers worldwide with strong moat and recurring revenue. " * 12)
    # ensure >50 tokens distinct chunks
    fake_chunks = [
        {"text": long_text + " Segment A unique details about iPhone sales and services growth across geographies."},
        {"text": long_text + " Segment B unique details about Mac, iPad, wearables and supply chain resilience and R&D."},
        {"text": long_text + " Segment C unique details about competitive landscape, regulatory risk and management quality."},
    ]
    monkeypatch.setattr(mod, "load_v2_chunks", lambda ticker: fake_chunks)
    monkeypatch.setattr(mod, "load_extracted_text", lambda ticker: ("",""))
    res = mod.process_ticker({"ticker":"AAPL","sector":"Technology","company":"Apple Inc","cik":"320193"})
