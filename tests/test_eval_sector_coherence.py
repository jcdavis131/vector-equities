...
def load_module():
    mod_path = pathlib.Path("/home/hatch/workspace/vector-equities/pipeline/eval_sector_coherence.py")
...
def test_smoke():
    mod = load_module()
    for fn in ["l2_normalize","knn_indices","purity_from_neighbors","random_purity_expectation","compute_report"]:
...
def test_compute_report_empty_tmp(tmp_path):
    ...
    data = {"points": [
        {"ticker":"AAPL","sector":"Technology","emb": (np.random.randn(8).tolist())},
        {"ticker":"MSFT","sector":"Technology","emb": (np.random.randn(8).tolist())},
        {"ticker":"JPM","sector":"Financials","emb": (np.random.randn(8).tolist())},
    ]}
