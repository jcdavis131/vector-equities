#!/usr/bin/env python3
"""
Thin wrapper — vector-equities operator MLOps CLI
...
def main():
    import argparse
    ap=argparse.ArgumentParser(description="vector-equities MLOps operator (scout/mlops-operator lane3)")
    ...
    args=ap.parse_args()
    if args.cmd=="fetch":
        print(fetch_caches("equities"))
    elif args.cmd=="train":
        if args.heavy or args.epochs>=60:
            print(train_heavy_handoff_entry("equities", epochs=args.epochs, dim=args.dim))
        else:
            print(train_smoke("equities", epochs=args.epochs, dim=args.dim))
    elif args.cmd=="eval":
        print(eval_gates("equities"))
    elif args.cmd=="export":
        print(export_assets("equities", onnx=args.onnx, wasm=args.wasm, pca=args.pca))
    elif args.cmd=="promote":
        print(candidate_promote("equities"))
    elif args.cmd=="ship":
        from bigbang.plugins.vector.operator import ship_vercel
        print(ship_vercel("equities"))
    else:
        ap.print_help()
